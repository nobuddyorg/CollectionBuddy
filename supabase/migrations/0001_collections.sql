...
on public.items for update using (user_id = auth.uid());
...
on public.item_categories for select using (user_id = auth.uid());

create policy "insert own item_categories"
on public.item_categories for insert with check (
  user_id = auth.uid()
...
on public.item_categories for delete using (user_id = auth.uid());
