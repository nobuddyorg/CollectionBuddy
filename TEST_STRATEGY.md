...
- **Placement**: a `ci.yml` job (`zap_baseline`), gated on `web` changing via the `changes` job — same gating as `build_and_test`/`mutation_test`/`lighthouse`, since this measures the built web bundle's runtime surface, not database behaviour. Not a `prek` hook — it needs a full build and a served export, the same cost class as the e2e suite.
