...
`.github/workflows/zap-baseline.yml` runs an [OWASP ZAP](https://www.zaproxy.org/) **baseline (passive-only) scan** — spider plus passive rules, never an active scan — against an isolated, ephemeral copy of the built static export, served on the runner itself. It is deliberately a separate workflow from `ci.yml`, not a new job inside it, so its thresholds can be tightened over time without touching the required PR gate.
...
- **Never scans anything but a local, ephemeral build.** The target is always `http://127.0.0.1:4173/`, an export built and served inside the same job — never `pages-deploy.yml`'s deployed URL. CLAUDE.md's guardrail against pointing signed-in or write-capable tooling at production applies here too, and this workflow has no path to production at all, signed in or out.
- **Scope is the signed-out surface** (the login page, the static shell, the manifest/service worker) — the same boundary `web/e2e/public/` already draws, and for the same reason: there is no username/password auth to script (Google OAuth only in production), and inventing one would test a login mechanism the app doesn't have. An authenticated pass, if ever wanted, could piggyback on local demo-mode anonymous sign-in against a local Supabase stack — an explicit future decision, not part of this scan.
...
- **Placement**: CI-only, gated on `web/**` (or the workflow/rules file itself) changing on PRs to `main`, plus `workflow_dispatch` for an on-demand run. Not a `prek` hook — it needs a full build and a served export, the same cost class as the e2e suite.
