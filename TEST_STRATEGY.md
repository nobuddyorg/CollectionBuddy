...
- **Failure mode**: no `--error` flag, so a finding surfaces as a SARIF upload to the code-scanning Security tab (`github/codeql-action/upload-sarif`, category `opengrep` so it can't collide with CodeQL's own entries there) without failing the job — the same split CodeQL's own default setup uses between "found something" and "the analysis itself broke".
...
- **Initial findings, all triaged**: 3, all false positives, all resolved by a small rewrite rather than a suppression — `.semgrepignore` carries no rule-specific entries as a result:
  - `javascript.lang.security.audit.unsafe-formatstring.unsafe-formatstring` on `web/scripts/mutation-summary.mjs`'s `` console.error(`Could not read ${REPORT_PATH}:`, error) ``. `REPORT_PATH` is a local constant, not attacker input, so there was nothing to fix security-wise — but the rule's own suggestion (a literal format string, the value passed separately: `console.error('Could not read %s:', REPORT_PATH, error)`) is genuinely the more idiomatic form for exactly the pattern the rule flags, so that's what shipped.
  - `javascript.lang.security.audit.prototype-pollution.prototype-pollution-loop.prototype-pollution-loop` on `web/src/app/i18n/I18nProvider.tsx`'s `resolveTranslationKey`, which walks a dot-separated key through a nested object with `value = value[k]` in a loop. Not exploitable as flagged — the loop only *reads*, it never assigns through `value[k]`, so there's no path from an attacker-controlled key to a mutated `Object.prototype`. Still, the existence check it used, `k in value`, matches *inherited* properties too (`'toString' in {}` is `true`), which is the shape this class of rule is generally right to be suspicious of; swapped for `Object.hasOwn(value, k)`, which only matches the translation JSON's own keys — tighter, and it no longer resembles the flagged pattern at all.
  - `javascript.lang.security.audit.detect-non-literal-regexp.detect-non-literal-regexp` on `web/src/app/i18n/parity.test.ts`'s `extractCallLiterals`, which built `` new RegExp(`\\b${name}\\(`, 'g') `` from a `name` parameter. That parameter only ever took two hardcoded values (`'t'`, `'tCount'`) at its two call sites — never file content, never anything external — so there was no ReDoS path, but the fix the rule's own message suggests (hardcode the regex) was still a straightforward, equivalent rewrite: the function now takes the `RegExp` itself, built as a literal at each call site.
...
- **Thresholds set from a measured baseline, with real margin, not the plugin's generic numbers**: measured locally (`chrome-launcher` against Playwright's own Chromium, mobile emulation, simulated throttling — Lighthouse CI's default collection profile) across several repeated 3-run `lhci autorun` invocations of the signed-out page, performance scored 0.82–0.93 and LCP landed around 3.0–3.2s each time; `categories:performance` is set to `0.8` and `largest-contentful-paint` to `5000`ms, a deliberate margin below even the lowest clean measurement rather than the median, so ordinary run-to-run variance on a CI runner doesn't flap the gate. The signed-in page's thresholds (`0.75` / `7000`ms) are set more conservatively still, since the sandbox this change was developed in has no working Docker daemon and so couldn't run a local Supabase stack to measure it directly — treat these two as the ones most likely to need tightening once real numbers come back from the first several CI runs, not as a permanently loose gate.
...
| `opengrep` | same as `e2e_local_stack` (its scanned tree spans `web/` and `supabase/`) | Semgrep's community rule pack finds nothing new in the scanned tree; results land in code scanning regardless of job status |
